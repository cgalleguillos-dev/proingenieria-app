...
import { FormActivity, INFERENCES, generateTimeOptions, PersonalOptions, timeOptionsAM, timeOptionsPM, supervisors, divers, getUsersByJob } from "@/constants";
                    ...
                    flex flex-row">
                        ...
                        >
                            ...
                            <TableBody items={PersonalOptions} emptyContent={"No hay personal"}>
                                {
                                    PersonalOptions.map((personal, index) => (
                                        <TableRow key={index} className="">
                                            <TableCell className="w-1/6 px-0">
                                                <div className="flex flex-row items-center justify-between">
                                                    ...
                                                    <Select
                                                        items={
                                                            getUsersByJob(personal.role).map((user, index) => (
                                                                { key: user.id, data: { name: user.name } }
                                                            ))
                                                        }
                                                        size="sm"
                                                        variant="bordered"
                                                    ...
                                                    >
                                                        {
                                                            (user) => (
                                                                <SelectItem key={user.key} value={user.key}>{user.data.name}</SelectItem>
                                                            )
                                                        }
                                                    </Select>
                        ...
                        <TableBody items={activities} emptyContent={"No hay actividades"}>
                            {
                                activities.map((activity, index) => (
                                    <TableRow key={index} className="w-96">
                                                ...
                                                <PopoverTrigger>
                                                    <Button size="lg"
                                                        ...
                                                        className="w-full text-left">{
                                                            activity.description ? activity.description : "Ingresa una Descripción"
                                                        }</Button>
                                                </PopoverTrigger>
                                                        ...
                                                        <div className="px-1 py-2 w-full h-full">
                                                            <div className="w-full h-full">
                                                                <Textarea
                                                                    ...
                                                                    onChange={(e) => setActivities([...activities.slice(0, index), { ...activity, description: e.target.value }, ...activities.slice(index + 1)])}
                            ...
                            <div className="flex flex-row w-full justify-end">
                                <div className="flex flex-row gap-2 justify-end">
                                    <Button size="sm" color="danger"
                                        ...
                                        onClick={() => { deleteLastInference(); }}>
                                        <TiDelete size={25} />
                                        ELIMINAR
                                    </Button>
                                    <Button size="sm" color="primary"
                                        onClick={() => { addInferenceRow(); }}>
                            ...
                            {inferences.map((inference, index) => (
                                <TableRow key={index} className="w-full">
                                    <TableCell className="w-1/6 px-0">
                                        ...
                                        >
                                            {timeOptions.map((time, index) => (
                                                <SelectItem key={time} value={index}>{time}</SelectItem>
                                            ))}
                                        </Select>
                                    </TableCell>
                                    ...
                                    <TableCell className="w-2/6">
                                        ...
                                        >
                                            {INFERENCES.map((inference, index) => (
                                                <SelectItem key={index} value={inference.id}>
                                        ...
                                        <Popover placement="right" showArrow offset={10}>
                                            <PopoverTrigger>
                                                <Button size="lg"
                                                    radius="sm"
                                                    className="w-full text-left">{
                                                        inference.description ? inference.description : "Ingresa una Descripción"
                                                    }</Button>
                                            </PopoverTrigger>
                                            <PopoverContent className="w-96">
                                                {(titleProps) => (
